...
void thresh_callback(int, void* )
{
    ...
    src_grayyy = imagePreparation::erosionImage(src_grayyy, 2, 2); // make a erosion to eliminate the noise
    src_grayyy = imagePreparation::dilationImage(src_grayyy, 2, 2); // make a erosion to eliminate the noise
    Canny( src_grayyy, canny_output, 125, 125*2, 3 );
    ...
    drawContours( drawing, contours, thresh, color, 2, 8, hierarchy, 0, Point() );
...
//    cv::waitKey(0); // Wait for a keystroke in the window
    ...
    src = imread("D:\\Facultate\\c++Project\\opencv_c-c-\\edgeDetection\\database\\attempt7\\pion\\pion_result_01.jpg",0);

    /// Convert image to gray and blur it
    blur( src, src_grayyy, Size(3,3) );

    /// Create Window
    char* source_window = (char *) "Source";
